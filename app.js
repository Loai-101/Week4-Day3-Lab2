...
let formattedNames = people.map(person => person.split(', ').reverse().join(' '));
// check if the result is correct
console.log('Exercise 5 my result: ', formattedNames);
...
let isAdultPresent = devs.some(dev => dev.year >= 18);
// check if the result is correct
...
let isEveryone19OrOlder = devs.every(dev => dev.year >= 19);
// check if the result is correct
